...
from _common import get_target, write_encrypted, http_get, load_wordlist, _read_encrypted, crt_sh_subdomains, wayback_subdomains
...
def _crt_sh(domain: str) -> set[str]: return crt_sh_subdomains(domain)
def _wayback(domain: str) -> set[str]: return wayback_subdomains(domain, limit=5000)
...
def _securitytrails(domain: str) -> set[str]:
    ...
    try:
        ...
        print(f"  [SecurityTrails] 错误: {e}", file=sys.stderr)
...
def _urlscan(domain: str) -> set[str]:
    ...
    try:
        ...
        print(f"  [URLScan] 错误: {e}", file=sys.stderr)
...
def _dnsdumpster(domain: str) -> set[str]:
    ...
    try:
        ...
        print(f"  [DNSDumpster] 错误: {e}", file=sys.stderr)
...
def _shodan(domain: str) -> set[str]:
    ...
    for query in (f"hostname%3A{domain}", f"ssl%3A{domain}"):
        ...
        try:
            ...
            print(f"  [Shodan] 错误: {e}", file=sys.stderr)
...
def _censys(domain: str) -> set[str]:
    ...
    try:
        ...
        print(f"  [Censys] 错误: {e}", file=sys.stderr)
...
def _otx(domain: str) -> set[str]:
    ...
    try:
        ...
        print(f"  [OTX] 错误: {e}", file=sys.stderr)
...
def _virustotal(domain: str) -> set[str]:
    ...
    try:
        ...
        print(f"  [VT] 错误: {e}", file=sys.stderr)
...
def _rapiddns(domain: str) -> set[str]:
    ...
    try:
        ...
        print(f"  [RapidDNS] 错误: {e}", file=sys.stderr)
...
def _riddler(domain: str) -> set[str]:
    ...
    try:
        ...
        print(f"  [Riddler] 错误: {e}", file=sys.stderr)
...
def _fofa(domain: str) -> set[str]:
    ...
    try:
        ...
        print(f"  [Fofa] 错误: {e}", file=sys.stderr)
...
def _hunter(domain: str) -> set[str]:
    ...
    try:
        ...
        print(f"  [Hunter] 错误: {e}", file=sys.stderr)
...
def main() -> int:
    ...
    # 合并 subdomain_enum.py 的结果 (如果存在)
    try:
        sub_data = _read_encrypted("subdomains")
        if sub_data:
            enum_subs = sub_data.get("unique_subdomains", [])
            if enum_subs:
                print(f"\n[passive] 合并 subdomain_enum 结果: {len(enum_subs)} 条", file=sys.stderr)
                all_candidates.update(enum_subs)
    except Exception:
        pass
