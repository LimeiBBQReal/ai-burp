...
def main() -> int:
    ...
    # 限制 URL 数量防止超时
    max_urls = int(os.environ.get("PARAM_MAX_URLS", "30"))
    max_requests = int(os.environ.get("PARAM_MAX_REQUESTS", "3000"))
    ...
    print(f"[param] 参数: {len(custom_params)}, 目标 URL: {len(test_urls)}, 最大请求: {max_requests}", file=sys.stderr)
    ...
    for url in test_urls:
        if request_count >= max_requests:
            print(f"  [param] 达到最大请求数 {max_requests}, 停止", file=sys.stderr)
            break
        with ThreadPoolExecutor(max_workers=20) as ex:
            futs = {ex.submit(check_param, url, p): p for p in custom_params}
            for fut in as_completed(futs):
                request_count += 2  # 每个参数测试 2 次请求
                if request_count >= max_requests:
                    break
                r = fut.result()
                if r:
