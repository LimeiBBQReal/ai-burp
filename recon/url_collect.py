...
def main() -> int:
    ...
    # verify_subdomains schema: {"verified_subdomains": {"domain.com": {"verified": ..., "ip": ...}}}
    verified_data = vdata.get("verified_subdomains", {})
    subdomains: list[str] = []
    if isinstance(verified_data, dict):
        subdomains = list(verified_data.keys())
    if not subdomains:
        # 兼容旧 schema
        subdomains = list(vdata.get("subdomains", {}).keys()) if isinstance(vdata.get("subdomains"), dict) else list(vdata.get("subdomains", []))
    ...
    deep_subs: list[str] = []
    try:
        dd = _read_encrypted("deep_subdomains")
